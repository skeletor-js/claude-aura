import { readFileSync, writeFileSync, mkdirSync, existsSync } from "fs";
import { join } from "path";
import { homedir } from "os";
import type { AuraConfig } from "../types.js";

export const CONFIG_DIR = join(homedir(), ".claude-aura");
export const CONFIG_PATH = join(CONFIG_DIR, "config.json");
export const PID_PATH = join(CONFIG_DIR, "daemon.pid");

export function ensureConfigDir(): void {
  if (!existsSync(CONFIG_DIR)) {
    mkdirSync(CONFIG_DIR, { recursive: true });
  }
}

export function loadConfig(): AuraConfig {
  if (!existsSync(CONFIG_PATH)) {
    throw new Error(
      `Config not found at ${CONFIG_PATH}. Run "claude-aura setup" first.`
    );
  }
  return JSON.parse(readFileSync(CONFIG_PATH, "utf-8")) as AuraConfig;
}

export function saveConfig(config: AuraConfig): void {
  ensureConfigDir();
  writeFileSync(CONFIG_PATH, JSON.stringify(config, null, 2) + "\n");
}

export function configExists(): boolean {
  return existsSync(CONFIG_PATH);
}
